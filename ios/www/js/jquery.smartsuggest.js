            ...
            if (event.keyCode != 13 && event.keyCode != 9) {
                ...
                if (q == '' || q.length < options.minChars) {
                    ...
                    timeout = setTimeout(function () {
                        ...
                        $.getJSON(options.src + getStart + "field_id="+options.target_id+"&q=" + q, function (data, textStatus) {
                            ...
                            if (textStatus == 'success') {
                                ...
                                if (!has_data) {
                                    ...
                                    $.each(data, function (i, group) {
                                        ...
                                        if (options.showEmptyCategories || (!options.showEmptyCategories && group['data'].length != 0)) {
                                            ...
                                            //console.log(fill_code);
                                            $.each(group['data'], function (j, item) {
                                                if (count < limit) {
                                                    // Build the link opening tab.
                                                    //escape out apostrophe's
                                                    var fillBoxText = item[options.fillBoxWith].replace(/'/g, "\\'");

                                                    var link_open = '<a href="';
                                                    link_open += (item['url'] != undefined) ? item['url'] : 'javascript: void(0);';
                                                    link_open += '" ';
                                                    link_open += (item['onclick'] != undefined) ? ' onclick="' + fill_code.replace("%", fillBoxText) + (options.executeCode ? item['onclick'] : '') + '" ' : '';
                                                    link_open += '>';
                                                    ...
                                                    output += (item['image'] != undefined && options.showImages && item['background'] == undefined) ? '<td width="40"><img src="' + item['image'] + '" /></td>' + "\n" : '';
                                                    output += (item['background'] != undefined && options.showImages) ? '<td width="40" style="background-color:'+item['background']+' "><img src="' + item['image'] + '" /></td>' + "\n" : '';
                                                    output += '<td>';
                                                    output += '<p>';
                                ...
                                boxObj.css('top', inputObj.offsetParent().top + inputObj.outerHeight());
                                boxObj.css('left', inputObj.offsetParent().left);
